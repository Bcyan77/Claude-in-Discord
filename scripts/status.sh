...
if [ -f "$GF" ] && [ -x "$PY" ]; then
    ...
    tline="$("$PY" -c "import json; d=json.load(open('$GF',encoding='utf-8')); t=[(e.get('options') or {}).get('tone') for e in d.values()]; t=[x for x in t if x]; r=t.count('strong')+t.count('raw'); o=t.count('off'); print(f'{len(t)}개 서버 설정 — 캐릭터 말투 덮음 {r}개 · 끔 {o}개' if t else '기본값 보통 (캐릭터 말투 유지 · 서버 어휘·화제만)')" 2>/dev/null)"
