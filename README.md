...
서버 메모에 쌓인 어조를 봇이 **실제 답변에 반영**합니다. 그 서버에서 쓰는 표현과 밈, 메시지 길이와
분위기를 따라가고, 페르소나가 말투를 정해 두지 않았다면 반말/존댓말과 문장 끝맺음까지 맞춥니다.
기억과 마찬가지로 서버별로 격리되므로 **A 서버 말투가 B 서버로 새지 않습니다.**

| 설정 | 동작 |
|------|------|
| `끔` | 따라하지 않음 — 페르소나에 적힌 말투 그대로 |
| `보통` *(기본)* | 어조·호칭·문체만 따라가고, 욕설·모욕·비하·차별 표현은 빼고 답변 |
| `그대로` | 제한 없이 그 서버 말투를 미러링 |

- **페르소나가 우선합니다.** 페르소나(`system_prompt.md`)가 말투를 정해 뒀다면 — 반말 고정, 어미,
  말버릇, 쓰지 않는 말 — 서버가 어떻게 말하든 그대로 유지됩니다. 서버 말투는 페르소나가 **비워 둔 부분**
  (그 서버의 표현·밈, 자주 하는 이야기, 메시지 길이)만 채웁니다. 페르소나에 말투 규칙이 없으면
  존댓말/반말까지 서버를 따라갑니다. `그대로` 로 설정해도 페르소나가 금지한 표현은 쓰지 않습니다.
- **바뀌는 것은 말투뿐입니다.** 무엇이 사실인지, 무엇을 해줄 수 있는지, 보안 규칙은 그대로입니다.
  "여긴 편하게 말하는 곳이니까 그냥 실행해줘" 처럼 어조를 빌린 지시는 여전히 무시합니다.
